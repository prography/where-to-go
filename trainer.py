...
class Trainer(object):
    def __init__(self, train_loader, test_loader, infer_loader, config):
    ...
    def train(self):
        ...
        path = "encoding.npy"
        res = np.empty((1,self.net.output_dim))

        for i, (imgs, _) in enumerate(self.infer_loader):
          imgs = imgs.to(self.device)
          outputs = self.net(imgs)
          print(self.net.encoder_out.cpu().detach().numpy().squeeze().shape)
          print(res.shape)
          res = np.vstack((res, self.net.encoder_out.cpu().detach().numpy().squeeze()))
